...
checkRepoExistsCmd="curl -s -o /dev/null -w \"%{http_code}\" --request GET $githubAPIUrl/repos/$org/$repoName --header 'Authorization: Basic $token'"
...
if [[ "$checkRepoExists" != "200" ]]; then
  ...
  # Create the Repo
  createRepoCmd="curl -s -o /dev/null -w \"%{http_code}\" --request POST '$githubAPIUrl/orgs/$org/repos' --header 'Accept: application/vnd.github.v3+json' --header 'Authorization: Basic $token' --header 'Content-Type: application/json' --data-raw '{ \"name\":\"Test-Repo\", \"private\": false, \"auto_init\": true, \"description\": \"This is a test repository for github api testing and R&D\"}'"
...
latestCommitCmd="curl --request GET '$githubAPIUrl/repos/$org/$repoName/git/refs/heads/master' --header 'Accept: application/vnd.github.v3+json' --header 'Authorization: Basic $token'"
...
# Create new branch
createBranchCmd="curl -s -o /dev/null -w \"%{http_code}\" --request POST '$githubAPIUrl/repos/$org/$repoName/git/refs' --header 'Accept: application/vnd.github.v3+json' --header 'Authorization: Basic $token' --header 'Content-Type: application/json' --data-raw '{ \"ref\": \"refs/heads/$branchName\", \"sha\": \"$latestCommitSHA\" }'"
...
# Checking in files
curl -i -X PUT -H "Authorization: Basic $token" -d "{\"path\": \"temp.txt\", \
